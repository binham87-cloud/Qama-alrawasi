...
import { reportingToStorageMonthKey, toReportingMonthKey } from "./domain/month_keys.mjs";
...
async function loadCanonicalState(db, monthKey, profile, uid) {
  ...
  const reportingKey = toReportingMonthKey(monthKey, "auto");
  let storageKey;
  try { storageKey = reportingToStorageMonthKey(reportingKey); } catch { storageKey = null; }
  state.legacyMonthOpenings = merge(
    await by(db, COLLECTIONS.legacyMonthOpenings, "monthKey", reportingKey),
    storageKey ? await by(db, COLLECTIONS.legacyMonthOpenings, "monthKey", storageKey) : [],
  );
  state.legacyOpeningStates = merge(
    await by(db, COLLECTIONS.legacyOpeningStates, "monthKey", reportingKey),
    storageKey ? await by(db, COLLECTIONS.legacyOpeningStates, "monthKey", storageKey) : [],
  );
  state.reconstructionPlans = merge(
    await by(db, COLLECTIONS.reconstructionPlans, "monthKey", reportingKey),
    storageKey ? await by(db, COLLECTIONS.reconstructionPlans, "monthKey", storageKey) : [],
  );
  state.monthAuthorities = merge(
    await by(db, COLLECTIONS.monthAuthorities, "monthKey", reportingKey),
    storageKey ? await by(db, COLLECTIONS.monthAuthorities, "monthKey", storageKey) : [],
  );
  // Normalize authority monthKey to reporting form so selectors match the request monthKey.
  state.monthAuthorities = state.monthAuthorities.map((a) => ({ ...a, monthKey: reportingKey }));
  state.legacyCycleOpenings = merge(
    await by(db, COLLECTIONS.legacyCycleOpenings, "monthKey", reportingKey),
    storageKey ? await by(db, COLLECTIONS.legacyCycleOpenings, "monthKey", storageKey) : [],
  );
  state.cycles = await by(db, COLLECTIONS.cycles, "reportingMonth", reportingKey);
  // Familiar months/{YYYY_M} document — KPI compatibility when cycles are empty.
  if (storageKey) {
    const monthSnap = await db.collection("months").doc(storageKey).get();
    if (monthSnap.exists) {
      const monthDoc = monthSnap.data() || {};
      state.legacyMonthData = monthDoc.data || null;
      const [y, m0] = storageKey.split("_").map(Number);
      state.legacyMonthYear = y;
      state.legacyMonthIndex0 = m0;
    }
  }
...
export function buildCanonicalReadModel(db) {
  return onCall({ enforceAppCheck: false }, async (request) => {
    ...
    const reportingKey = toReportingMonthKey(monthKey, "auto");
    const { state, manager } = await loadCanonicalState(db, reportingKey, profile, request.auth.uid);
    ...
    const projection = manager
      ? managerMonthlyReport(state, reportingKey, asOfDate)
      : employeeOperationalReport(state, reportingKey, asOfDate);
    const operational = monthlyOperationalProjection(state, reportingKey, asOfDate);
    const legacyExpenseRows = operational.compatibilitySource === "legacy_month" ? (state.legacyExpenseRows || []) : [];
    const expenseRecords = manager
      ? [...(state.expenses || []), ...legacyExpenseRows]
