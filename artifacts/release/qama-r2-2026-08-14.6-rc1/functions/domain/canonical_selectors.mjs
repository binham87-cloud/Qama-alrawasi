...
import { projectLegacyMonthFinance } from "./legacy_month_projection.mjs";
...
export function monthlyOperationalProjection(state, monthKey, asOfDate) {
  ...
  let cards = { targetFils: openingValue(opening, "targetFils", "targetResidualFils") + sum(liveTargetDetails, "targetFils"), collectedFils: sum(collectedDetails, "amountFils"), depositedFils: sum(depositedDetails, "amountFils"), receivedNotDepositedFils: sum(cashDetails, "amountFils"), arrearsFils: sum(arrearsDetails, "remainingFils"), notYetDueFils: sum(notYetDueDetails, "remainingFils"), partialRemainingFils: sum(partialRemainingDetails, "remainingFils"), unallocatedOverpaymentFils, uncollectedAtEvictionFils: sum(evictionDetails, "amountFils") };
  let details = { target: targetDetails, collected: collectedDetails, deposited: depositedDetails, receivedNotDeposited: cashDetails, arrears: arrearsDetails, notYetDue: notYetDueDetails, partialRemaining: partialRemainingDetails, uncollectedAtEviction: evictionDetails };
  let compatibilitySource = null;
  // Compatibility KPIs from familiar months/{YYYY_M}:
  // 1) Empty cycles + no opening residuals → full legacy projection.
  // 2) CLEAN_START / evidence_only_zero months → keep legacy month KPIs even after
  //    ensureCompatibleCycle creates a single live cycle (otherwise Target collapses
  //    from full-month truth to one unit). New cashLots still drive holding.
  // Do NOT add live cycle targets on top of legacy (would double-count).
  const liveCycleTarget = sum(liveTargetDetails, "targetFils");
  const openingTarget = openingValue(opening, "targetFils", "targetResidualFils");
  const authority = (state.monthAuthorities || []).find((x) => x.monthKey === monthKey && x.status === "ACTIVE");
  const forceLegacyMonthKpis = authority?.legacyProjectionEffect === "evidence_only_zero";
  if (state.legacyMonthData && ((liveCycleTarget === 0 && openingTarget === 0) || forceLegacyMonthKpis)) {
    ...
    cards = {
      ...
      collectedFils: legacy.cards.collectedFils,
      depositedFils: legacy.cards.depositedFils,
      arrearsFils: legacy.cards.arrearsFils,
      // Keep canonical custody if cashLots exist; otherwise legacy leaves 0.
      receivedNotDepositedFils: cards.receivedNotDepositedFils || legacy.cards.receivedNotDepositedFils,
    };
    details = {
      ...details,
      target: legacy.details.target,
      collected: legacy.details.collected,
      deposited: legacy.details.deposited,
      arrears: legacy.details.arrears,
    };
    ...
    const canonicalDeposited = cards.depositedFils;
    const legacyDeposited = legacy.cards.depositedFils;
    const hasCanonicalDepositEvidence = (bankDeposits.length + cashDeposits.length) > 0
      || openingValue(opening, "depositedFils", "depositedResidualFils") > 0;
    const reconciliationExceptions = [];
    if (hasCanonicalDepositEvidence && legacyDeposited > 0 && legacyDeposited !== canonicalDeposited) {
      reconciliationExceptions.push({
        code: "DEPOSITED_DUAL_REPRESENTATION",
        classification: "E",
        canonicalFils: canonicalDeposited,
        legacyFils: legacyDeposited,
        resolution: "canonical_authoritative_legacy_transactions_preserved_unread_as_kpi",
      });
    }
    cards = {
      ...cards,
      ...
      // Do not Math.max competing ledgers. Canonical deposit events win; otherwise
      // legacy transactions remain compatibility-only until classified.
      depositedFils: hasCanonicalDepositEvidence ? canonicalDeposited : legacyDeposited,
    ...
    state.legacyExpenseRows = legacy.expenseRows;
    return { monthKey, asOfDate: date, cards, details, custodyByEmployee: custody.byHolder, openingState: opening ? { bootstrapBatchId: opening.bootstrapBatchId, classification: opening.classification, attributionStatus: opening.attributionStatus || null } : null, compatibilitySource, reconciliationExceptions };
  }
  return { monthKey, asOfDate: date, cards, details, custodyByEmployee: custody.byHolder, openingState: opening ? { bootstrapBatchId: opening.bootstrapBatchId, classification: opening.classification, attributionStatus: opening.attributionStatus || null } : null, compatibilitySource, reconciliationExceptions: [] };
}
...
export function managerMonthlyReport(state, monthKey, asOfDate) {
  ...
  const legacyExpenses = operational.compatibilitySource === "legacy_month" ? (state.legacyExpenseRows || []) : [];
  ...
  const expensesFils = expenses.reduce((s, x) => s + Number(x.amountFils || 0), 0) + legacyExpenses.reduce((s, x) => s + Number(x.amountFils || 0), 0);
