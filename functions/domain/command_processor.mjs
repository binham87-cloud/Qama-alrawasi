...
import { normalizeExpenseCategory } from "./legacy_month_projection.mjs";
...
function expenseCategory(payload) {
  const normalized = normalizeExpenseCategory(payload?.category || "operating");
  if (!normalized || !EXPENSE_CATEGORIES.includes(normalized)) throw new Error("EXPENSE_CATEGORY_INVALID");
  return normalized;
}
function expenseSubject(payload, category) {
  const subject = String(payload?.subject || payload?.unitId || payload?.facility || "").trim();
...
function rejectExpense(state, ctx) {
  assertManager(ctx.actor);
  const expense = get(state.expenses, ctx.payload.expenseId, "EXPENSE_NOT_FOUND");
  if (expense.status !== "pending") throw new Error("EXPENSE_NOT_PENDING");
  const reason = String(ctx.payload.reason || "").trim();
  const before = clone(expense);
  expense.status = "rejected";
  expense.rejectedBy = ctx.actor.id;
  expense.rejectedAt = nowIso(ctx);
  expense.rejectionReason = reason;
  expense.version = Number(expense.version || 1) + 1;
  audit(state, ctx, "expense_rejected", "expense", expense.id, before, expense);
  return { expenseId: expense.id, status: "rejected", financialEffectFils: 0 };
}
...
const HANDLERS = {
  ...
  requestExpense, approveExpense, rejectExpense, executeExpense, reverseExpense,
